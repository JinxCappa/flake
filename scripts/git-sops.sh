...
decrypt_repo() {
	...
	for ((i=0; i<${#files_to_decrypt[@]}; i+=batch_size)); do
		...
		# Remove from cache and checkout with error handling. Checkout invokes
		# smudge (not clean), which decrypts each file into the working tree.
		if ! git rm --cached --quiet "${batch[@]}" 2>/dev/null; then
			echo "Warning: Failed to remove some files from cache in batch $batch_num" >&2
		fi

		if ! git checkout HEAD --quiet -- "${batch[@]}" 2>/dev/null; then
