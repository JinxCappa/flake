...
in
{
  ...
  environment.etc = {
    "nixos/omni/omni.asc".source = ./files/crypt/omni.asc;
    "nixos/docker-compose.yaml".source = files/crypt/docker-compose.yaml;
  };

  environment.variables = {COMPOSE_FILE = "/etc/nixos/docker-compose.yaml";};
  ...
  services = {
    cron = {
      ...
      systemCronJobs = [
        ...
        "@daily nixos ${pkgs.docker}/bin/docker exec omni-etcd-backup backup"
      ];
    };
    incron = {
      enable = true;
      systab = ''
        /etc/acme/certs/omni.${toml.domain}_ecc/fullchain.cer IN_CLOSE_WRITE ${pkgs.docker}/bin/docker restart omni
      '';
    };
  };
}